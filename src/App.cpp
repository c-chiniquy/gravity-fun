...
void App::Run()
{
	if (context.Load(
		...
		ig::RenderSettings
		{
			...
			.maxFramesInFlight = numFramesInFlight,
...
void App::Start()
{
	...
	LoadSizeDependantResources(context.GetBackBufferExtent());
	...
	// ImGui
	{
		...
		ig::DescriptorHeap& heap = context.GetDescriptorHeap();
		imguiDescriptor = heap.AllocatePersistent(ig::DescriptorType::Texture_SRV);
		ImGui_ImplDX12_Init(
			context.GetD3D12Device(),
			context.GetNumFramesInFlight(),
			ig::GetFormatInfoDXGI(context.GetBackBufferRenderTargetDesc().colorFormats[0]).dxgiFormat,
			heap.GetD3D12DescriptorHeap_Resources(),
			heap.GetD3D12CPUHandle(imguiDescriptor),
			heap.GetD3D12GPUHandle(imguiDescriptor));
...
void App::LoadSizeDependantResources(ig::Extent2D backBufferSize)
{
...
void App::ApplyScenario(Scenario scenario)
{
	particleRenderer.ClearHeatmap();
	universe.ClearAllParticles();

	ScenarioDesc desc = GenerateScenarioDesc(scenario, context.GetBackBufferExtent());
...
void App::OnLoopExited()
{
	context.WaitForIdleDevice();

	if (!imguiDescriptor.IsNull()) context.GetDescriptorHeap().FreePersistent(imguiDescriptor);
	imguiDescriptor.SetToNull();
...
void App::Update(double elapsedSeconds)
{
		...
		if (ImGui::BeginTabBar("Tab0", 0))
		{
			if (ImGui::BeginTabItem("Galaxies"))
			{
				...
				if (ImGui::Button("Generate 1 galaxy"))
				{
					...
					ParticleState state = GenerateCenteredGalaxy(inputParams.galaxy[0], inputParams.ringParam,
						universe.GetPhysicsParams(), context.GetBackBufferExtent());
				...
				if (ImGui::Button("Generate 2 galaxies"))
				{
					...
					ParticleState state = GenerateCollidingGalaxies(inputParams.galaxy, 2, inputParams.colParam,
						inputParams.ringParam, universe.GetPhysicsParams(), context.GetBackBufferExtent());
					universe.SetParticleState(std::move(state));
				}
				ImGui::EndTabItem();
			}
			if (ImGui::BeginTabItem("NBody"))
			{
				...
				if (ImGui::Button("Generate lined particles", ImVec2(204, 0)))
				{
					particleRenderer.ClearHeatmap();
					universe.ClearAllParticles();
					ParticleState state = GenerateLinedParticles(inputParams.linedParam, false, context.GetBackBufferExtent());
					universe.SetParticleState(std::move(state));
				}
				...
				if (ImGui::Button("Generate random particles", ImVec2(204, 0)))
				{
					particleRenderer.ClearHeatmap();
					universe.ClearAllParticles();
					ParticleState state = GenerateRandomMassParticles(inputParams.randParam, context.GetBackBufferExtent());
					universe.SetParticleState(std::move(state));
				}
			...
			if (ImGui::BeginTabItem("Physics"))
			{
				...
				ImGui::SetItemTooltip(
					"Warning: Disabling compute shaders will drop FPS significantly\n"
					"if simulating a large number of particles!");
				...
				if (ImGui::Button("Generate visual particles for cursor interaction"))
				{
					...
					ParticleState state = GenerateLinedParticles(linedVisualParams, true, context.GetBackBufferExtent());
					universe.SetParticleState(std::move(state));
				}
			...
			{
				std::string str = ig::ToString("iglo v" IGLO_VERSION_STRING " " IGLO_GRAPHICS_API_STRING);
	...
	cmd.Begin();
	{
		...
		if (nothingToDraw)
		{
			cmd.AddTextureBarrier(context.GetBackBuffer(),
				...
				ig::BarrierLayout::RenderTarget, ig::BarrierLayout::Present);
		}
		else
		{
